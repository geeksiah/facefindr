...
async function insertProfileWithSchemaFallback(
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    const result = await adminDb.from(table).insert(attemptPayload);
  ...
  return adminDb.from(table).insert(attemptPayload);
}
...
export async function register(formData: RegisterInput) {
  ...
  if (isCreatorUser(normalizedUserType)) {
    const { error: profileError } = await insertProfileWithSchemaFallback(adminDb, 'photographers', {
        id: authData.user.id,
        email: validated.data.email,
        display_name: validated.data.displayName,
    ...
    const { error: profileError } = await insertProfileWithSchemaFallback(adminDb, 'attendees', {
        id: authData.user.id,
        email: validated.data.email,
        display_name: validated.data.displayName,
        username: validated.data.username,
